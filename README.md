...
* 多账号轮询 + **失败换号重试直到成功** + 指数退避冷却；内置 FlareSolverr，一键启动；
* **配额可视化**：面板展示每个号 fast/auto 剩余额度（迷你进度条）+ 全池总量/分布直方图；
* **记录落盘**：所有搜索记录与完整结果写入 SQLite（`data/search.db`），重启不丢、历史不限量。
...
* **概览**：账号统计 + 平均首字/平均耗时/成功率 + 最近请求耗时趋势图（失败点标红）+
  **号池配额总览**（fast 剩余总量进度条 + 5 档分布直方图）；
* **账号池**：每个号显示 **fast/auto 配额迷你进度条**（绿>50%/黄≤50%/红=0）、多行 SSO
  批量导入（uid 自动解析）、删除、重载、手动「刷新配额」/冷却/忙碌状态；
* **搜索测试**：实时搜索并展示全部结果，点任意条目弹出完整内容（网页/帖子）；
* **请求日志**：SSE 实时推送 + 下滑无限加载历史（SQLite 落盘，可一直翻到最早一条）；
  只展示搜索词/结果数/首字/总耗时/状态，不展示任何密钥；点击任意一条 →
  右侧详情面板展示该次搜索的完整结果快照（重启后依然可查）；
...
```
src/
  index.ts   (~300 行) HTTP 入口：配置 + 路由 + 双认证 + SSE + 静态托管
  grok.ts    (~600 行) 上游三合一：号池 + 清关 + WebSocket 会话采集 + 帧解析 + 日志/结果快照
  quota.ts   (~200 行) 账号配额：直查上游 /rest/rate-limits + 落盘缓存 + 定时/顺带刷新
  db.ts      (~130 行) 搜索记录落盘：SQLite（logs + results），历史不限量
